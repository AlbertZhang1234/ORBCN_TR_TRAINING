...
- 上传接口先把 PDF/PNG/JPEG 写入服务器持久化目录，再创建 `otto_supplier_invoice_drafts` 记录；任务与附件持久化完成后，当前请求才尝试识别。
...
普通报销和供应商上传最终调用同一个发票识别客户端。供应商上传采用同步快速路径：任务落库后，当前上传请求按任务 ID 原子领取并立即识别；识别结果、可编辑抬头和行项目写回草稿后才随上传响应返回，因此页面可以直接展示已经持久化的结果。该过程不创建正式发票，也不改变后续“保存当前/保存全部”的业务边界。

若任务已被 Worker 领取、同步路径启动失败、请求进程退出或识别额度暂不可用，已保存的任务和原文件不会删除。Worker 通过同一领取和租约机制接管排队或租约过期任务；同步请求与 Worker 不会同时处理同一任务。页面收到 `queued` / `recognizing` 状态时继续按原方式刷新，收到 `ready` / `error` 时直接展示结果或错误。

Next.js Node 服务启动时由 `instrumentation.ts` 注册后台兜底轮询器。采用数据库 `FOR UPDATE SKIP LOCKED` 领取任务，每实例默认两个并发，任务完成后立即补充空闲槽位。管理员可在“系统设置 → 系统配置”调整上传、后台及总识别并发。识别客户端与普通报销共用数据库额度，仍调用现有 8201 服务，不新增模型服务。升级时需执行 `create_system_config.sql`。
...
若 `otto_system_config` 缺失或 `invoice_recognition` 配置行未初始化，Worker 会记录一次带
`SYSTEM_CONFIG_MISSING` / `system_config_missing` 的致命启动错误并立即停止，不会持续轮询写日志。
执行 `create_system_config.sql` 后需要重启 Next 服务，让 Worker 重新启动。其他临时数据库或存储错误仍按轮询策略重试。
...
```
npx tsx --test tests/supplier-upload-recognition.test.ts tests/supplier-draft-store.test.ts tests/supplier-invoice-recognition.test.ts tests/invoice-lines.test.ts
```
